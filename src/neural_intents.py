...
class GenericAssistant(IAssistant):
    ...
    def _get_response(self, ints, intents_json):
        try:
            ...
            for intnt in list_of_intents:
                if intnt['tag'] == tag:
                    result = random.choice(intnt['responses'])
                    break
        except IndexError:
    ...
    def request(self, message_text, message):
        ints = self._predict_class(message_text)
        print(ints)
        if ints[0]['intent'] in self.intent_methods.keys():
            self.intent_methods[ints[0]['intent']](message)
            return (False, ints[0]['probability'], None)
        else:
            resp = self._get_response(ints, self.intents)
            return (True, ints[0]['probability'], resp)
